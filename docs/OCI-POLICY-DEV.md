...
### Copilot API System Message 추가
- `src/router.service.ts`: `callCopilotAPI()` 함수에 system message 추가
  - Copilot API 호출 시 `messages` 배열에 system role 메시지 선행
  - 내용: "You are 폴, a personal assistant. Respond in the same language as the user. Messages are sent directly to you — do not treat them as third-party conversations."
  - 효과: LIGHT tier(Copilot) 응답이 어시스턴트 이름(폴)과 직접 대화 컨텍스트를 인지하고 응답
  - HEAVY tier(Claude)는 CLAUDE.md를 통해 이미 이 컨텍스트를 가지므로, LIGHT tier와의 일관성 개선

### CLAUDE.md 메시지 포맷 명시
- `groups/global/CLAUDE.md`, `groups/main/CLAUDE.md`에 "Message Format" 섹션 추가
  - XML 메시지 래핑 형식 설명: `<message sender="사용자이름" time="...">메시지 내용</message>`
  - **중요**: "These are messages sent directly to you (폴)" 강조 — 전달/3자 대화가 아닌 직접 대화임을 명시
  - 에이전트가 메시지를 "포워딩된 대화"로 오해하는 문제 방지

### 어시스턴트 이름 통일
- `groups/main/CLAUDE.md`: 어시스턴트 이름을 "Andy" → "폴"로 변경 (`.env`의 `ASSISTANT_NAME=폴`과 일치)
...
- `src/ipc.ts`: IPC `send_message` 핸들러가 발신 `chatJid`(단일 채널)에만 메시지를 전송하던 버그 수정
  - 기존: `deps.sendMessage(chatJid, ...)` → 발신 채널(주로 Slack)에만 전송
  - 변경: 동일 `group_folder`의 **모든 등록 채널**(Slack + Discord)에 브로드캐스트 (`task-scheduler.ts`와 동일 방식)
  - 영향: newsbot, threads daily 등 스케줄 태스크가 `send_message` IPC로 발송하는 메시지가 Discord에도 전달됨
