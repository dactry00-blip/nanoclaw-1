...
`openclaw.json`의 `tools.profile`이 에이전트가 사용할 수 있는 도구를 결정한다.

| 프로필 | 포함 도구 | 용도 |
|--------|----------|------|
| `coding` | read, edit, write, **exec**, process, cron, sessions_*, memory_*, web_search, web_fetch | **현재 사용 중** — 에이전트가 셸 명령 실행, 웹 검색 가능 |
| `messaging` | message, sessions_list, sessions_history, sessions_send, session_status | 메시지 송수신만 가능 (exec/read/write 없음) |

**⚠️ `messaging` 프로필 사용 시 에이전트가 exec 도구를 사용할 수 없어 Threads API 호출, 웹 검색 등이 불가능.**
...
2. `safeBins` (curl, node만 허용)
3. `safeBinProfiles` (curl의 파일 쓰기 플래그 차단: `-o`, `-O`, `-T`)
4. `safeBinTrustedDirs` (/usr/bin, /usr/local/bin의 바이너리만 신뢰)
